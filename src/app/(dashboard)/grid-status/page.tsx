            ...
            <div className="space-y-3">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs">
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-2">
                      ...
                      <div>
                        ...
                        <div className="font-semibold text-slate-900">{fillsStats.buys} / {fillsStats.sells}</div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="font-semibold text-slate-900">
                          {fillsStats.winTrades === undefined || fillsStats.lossTrades === undefined ? '—' : `${fillsStats.winTrades} / ${fillsStats.lossTrades}`}
                        </div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="font-semibold text-slate-900">{fillsStats.avgBuy === undefined ? '—' : `$${formatPrice(fillsStats.avgBuy)}`}</div>
                      </div>
                      <div>
                        <div className="text-slate-500">Avg sell</div>
                        <div className="font-semibold text-slate-900">{fillsStats.avgSell === undefined ? '—' : `$${formatPrice(fillsStats.avgSell)}`}</div>
                      </div>
                      <div>
                        <div className="text-slate-500">Realized PnL</div>
                        <div className="font-semibold text-slate-900">
                          {fillsStats.realizedPnl === undefined ? '—' : fillsStats.realizedPnl.toLocaleString(undefined, { maximumFractionDigits: 8 })}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs">
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-2">
                      {(() => {
                        ...
                        const winRate =
                          typeof ps?.winRate === 'number'
                            ? ps.winRate
                            : profitTrades + lossTrades > 0
                              ? profitTrades / (profitTrades + lossTrades)
                              : null;
                        ...
                        return (
                          <>
                            <div>
                              ...
                              <div className="font-semibold text-slate-900">
                                ${initial.toLocaleString(undefined, { maximumFractionDigits: 8 })}
                              </div>
                              <div className="text-[11px] text-slate-500">Start: {formatPrice(startP)}</div>
                            </div>
                            <div>
                              <div className="text-slate-500">Current value</div>
                              <div className="font-semibold text-slate-900">
                                ${currentVal.toLocaleString(undefined, { maximumFractionDigits: 8 })}
                              </div>
                              <div className="text-[11px] text-slate-500">Now: {formatPrice(curP)}</div>
                            </div>
                            <div>
                              ...
                              <div className={`text-[11px] ${pnl >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                                {pnl >= 0 ? '+' : ''}${pnl.toLocaleString(undefined, { maximumFractionDigits: 8 })}
                              </div>
                            </div>
                          </>
                        );
                      ...
                      <div>
                        ...
                        <div className="font-semibold text-slate-900">
                          {(() => {
                            const ps = (bot as any).runtime?.paperStats;
                            const c = typeof ps?.closedTrades === 'number' ? ps.closedTrades : 0;
                            return c;
                          })()}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="font-semibold text-slate-900">
                          {(() => {
                            const ps = (bot as any).runtime?.paperStats;
                            const rp = typeof ps?.realizedPnl === 'number' ? ps.realizedPnl : null;
                            if (rp === null) return '—';
                            const cls = rp >= 0 ? 'text-emerald-700' : 'text-rose-700';
                            return (
                              <span className={cls}>
                                {rp >= 0 ? '+' : ''}
                                {rp.toLocaleString(undefined, { maximumFractionDigits: 8 })}
                              </span>
                            );
                          })()}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="font-semibold text-slate-900">
                          {(() => {
                            const ps = (bot as any).runtime?.paperStats;
                            const p = typeof ps?.profitTrades === 'number' ? ps.profitTrades : 0;
                            const l = typeof ps?.lossTrades === 'number' ? ps.lossTrades : 0;
                            const denom = p + l;
                            const wr = typeof ps?.winRate === 'number' ? ps.winRate : denom > 0 ? p / denom : null;
                            return wr === null ? '—' : `${(wr * 100).toFixed(1)}%`;
                          })()}
                        </div>
                      </div>
                      <div>
                        <div className="text-slate-500">Profit trades</div>
                        <div className="font-semibold text-slate-900">{(bot as any).runtime?.paperStats?.profitTrades ?? 0}</div>
                      </div>
                      <div>
                        <div className="text-slate-500">Loss trades</div>
                        <div className="font-semibold text-slate-900">{(bot as any).runtime?.paperStats?.lossTrades ?? 0}</div>
                      </div>
                    </div>
                    <div className="mt-2 text-[11px] text-slate-500">
                      Paper profit/loss is computed from simulated round-trips (entry → exit). Fees/slippage are not included.
                    </div>
                  </div>
                ) : null}
                ...
                {exec === 'live' ? (
                  <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs">
                    <div className="flex items-center justify-between gap-2">
                      <div className="font-semibold text-slate-700">Live notional PnL (qty ÷ leverage)</div>
                      <Badge tone="red">LIVE</Badge>
                    </div>
                    <div className="mt-2 grid grid-cols-3 gap-2">
                      {(() => {
                        ...
                        const qty = Number(cfg.quantity);
                        const units = lev > 0 ? qty / lev : NaN;
                        const startP = toNum(bot.runtime?.startedPrice);
                        const curP = typeof live === 'number' && Number.isFinite(live) ? live : null;
                        const has = Number.isFinite(units) && units > 0 && typeof startP === 'number' && curP !== null && startP > 0;
                        if (!has) {
                          return (
                        ...
                        const hasOpenPosition = Array.isArray((bot as any).runtime?.positions) && (bot as any).runtime.positions.length > 0;
                        if (!hasOpenPosition) {
                          return (
                            <div className="col-span-3 text-slate-500">
                              No trades yet — PnL will show after the first successful order opens a position.
                            </div>
                          );
                        }
                        const initial = units * startP;
                    ...
                    <div className="mt-2 text-[11px] text-slate-500">
                      This is a simple notional estimate from the bot’s configured quantity/leverage. Real PnL depends on actual fills and fees.
                    </div>
                  </div>
                ) : null}
              </div>
